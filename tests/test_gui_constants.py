...
class Test_스페이싱_상수:
    ...
    def test_구체적_패딩값(self):
        ...
        assert PADDING_XS == 4
        assert PADDING_SM == 8
        assert PADDING_MD == 12
        assert PADDING_LG == 16
        assert PADDING_XL == 20
        assert PADDING_2XL == 24
    ...
    def test_요소_간격(self):
        """요소 간 간격이 정의되어 있다."""
        assert ELEMENT_SPACING == PADDING_SM
...
class Test_윈도우_크기:
    ...
    def test_메인_영역_패딩(self):
        """메인 영역 패딩이 정의되어 있다."""
        assert MAIN_PADX == PADDING_MD
        assert MAIN_PADY == PADDING_MD
...
class Test_테두리_반지름:
    ...
    def test_기본_테두리_반지름(self):
        """기본 테두리 반지름이 8px이다."""
        assert BORDER_RADIUS_DEFAULT == 8
...
class Test_폰트_크기:
    ...
    def test_제목_폰트_크기(self):
        """제목 폰트 크기들이 정의되어 있다."""
        assert FONT_SIZE_H1 == 28
        assert FONT_SIZE_H2 == 24
        assert FONT_SIZE_H3 == 20
        assert FONT_SIZE_TITLE == 18

    def test_본문_폰트_크기(self):
        """본문 폰트 크기들이 정의되어 있다."""
        assert FONT_SIZE_BASE == 13
        assert FONT_SIZE_SM == 12
        assert FONT_SIZE_XS == 11
        assert FONT_SIZE_LG == 14
    ...
    def test_폰트_조합들(self):
        """주요 폰트 조합들이 정의되어 있다."""
        assert FONT_TITLE[1] == 20
        assert FONT_LABEL[1] == 13
        assert FONT_LABEL_BOLD[1] == 13
        assert FONT_SMALL[1] == 12
        assert FONT_SMALL_BOLD[1] == 12
...
class Test_상수_일관성:
    ...
    def test_메인_패딩이_표준_패딩(self):
        ...
        assert MAIN_PADX in (PADDING_SM, PADDING_MD, PADDING_LG)
        assert MAIN_PADY in (PADDING_SM, PADDING_MD, PADDING_LG)
